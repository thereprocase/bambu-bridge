...
export function mount(root, app) {
  ...
  const page = el('main', { class: 'app-main library-page' }, [
    ...
    search, status, content, loadMore,
  ]);
  ...
  function render() {
    ...
    for (const row of matches) {
      ...
      const projectState = !hasProject ? 'not included' : row.project_roundtrip_verified ? 'reopening verified' : 'stored; reopening not yet verified';
  ...
  async function load() {
    if (busy || !more) return;
    busy = true; loadMore.disabled = true;
    try {
    ...
    } finally { busy = false; loadMore.disabled = false; }
  }
